<script lang="ts">
	...
	async function saveUsername(nextValue: string) {
		...
		try {
			...
			viewerUsername = normalized;
			await goto(`/${normalized}`);
		} catch (error) {
			usernameError = error instanceof Error ? error.message : 'Unable to update username.';
		} finally {
	...
	async function init() {
		...
		try {
			const { data: rows, error: uerr } = await supabase
				.from('users')
				.select(
					'id, display_name, username, selected_block_hour, selected_block_half, active_day_date'
				)
			...
			const viewerRow = (rows ?? []).find((r) => (r.id as string | null) === viewerUserId);
			viewerUsername = (viewerRow?.username as string | null) ?? null;
		...
		{:else}
			...
			>
				...
				<span class="text-[15px] font-semibold tracking-wide text-stone-400">
					{$goalBarStore.currentRangeLabel}
				</span>
			</div>
		{/if}
	</div>
...
{#if showSettings}
	...
	<SettingsModal
		...
		{usernameSaving}
		{usernameError}
		onSaveUsername={(next) => saveUsername(next)}
		onLogout={async () => {
			try {
